...
Verify the created data is displayed
...
Click the Button or Navigate Back
    [Documentation]    Using this keyword we can click button 
    [Arguments]    ${element}    ${name}
...
create Platform OKR
    ...
    Press Keys    ${Input_Search_Manager}    ${Manager}
    ${Assign}    Format String    ${Select_okr_assignto}    name=${Manager}
...
Create Squad
    [Arguments]    ${name}    ${lead}    ${element}    ${DATA}    ${element1}        
    ...
    ${Assign}    Format String    ${Select_okr_assignto}    name=${lead}
    Wait Until Element is present then click the element    ${Assign}
    Wait Until Element is present then click the element    ${element} 
...
Edit squad product
    [Arguments]    ${name}    ${lead}        
    Click the Kebab button    ${BTN_EDIT_product_squad}    ${PPOS_SQUAD}    
...
Create PPG 
    ...
    ${SelectType}    Format String    ${Input_Select_Type}    name=${Type}
    Wait Until Element is present then click the element    ${SelectType}
    Wait Until Element is present then click the element    ${Element}
...
create,edit strategic outcome
    ...
    Wait Until Element is present then click the element    ${Drop_Down_SO_AssignTo}
    Press Keys    ${Input_Search_SO_Member}    ${manager}
    ${Selectmanager}    Format String    ${Select_So_Member}    name=${manager}
    Wait Until Element is present then click the element    ${Selectmanager}
    Wait Until Element is present then click the element    ${Element}

Create,edit OKR
    [Documentation]    Using this keyword we can create,edit OKR
    [Arguments]    ${name}    ${description}    ${ppg_product}    ${manager}    ${ppg_product_name}    ${Element}
    ...
    ${SelectAdd}    Format String    ${Select_Add_Under}    name=${ppg_product}
    Wait Until Element is present then click the element    ${SelectAdd}
    Wait Until Element is present then click the element    ${Drop_Down_Assign_To}
    Press Keys    ${Input_Search_SO_Member}    ${manager}
    ${Selectmanager}    Format String    ${Select_So_Member}    name=${manager}
    Wait Until Element is present then click the element    ${Selectmanager}
    Wait Until Element is present then click the element    ${Drop_Down_Select_ppg_product}
    ${Selectppgprod}    Format String    ${Select_ppg_product}    name=${ppg_product_name}
    Wait Until Element is present then click the element    ${Selectppgprod}
    Wait Until Element is present then click the element    ${Element}       
Create Key Results
    [Documentation]    Using this keyword we can edit Key Results
    [Arguments]    ${name}    ${description}    ${edit baseline}    ${edit UOM}    ${manager}
    Press Keys    ${INPUT_Key_Result_Name}    CTRL+A+BACKSPACE
    Input Text    ${INPUT_Key_Result_Name}    ${name}
    Press Keys    ${INPUT_Key_Result_Description}    CTRL+A+BACKSPACE
    Press Keys    ${INPUT_Key_Result_Description}    ${description}
